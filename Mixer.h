...
public:
    ...
    void setChannelOutputPair (MixerChannel ch, int pair) { channels[idx (ch)].outputPair.store (pair, std::memory_order_relaxed); }
                  ...
                  int numSamples)
    {
        ...
        struct Snapshot { float gain; bool silenced; int pair; };
        ...
        for (int c = 0; c < kNumMixerChannels; ++c)
        {
            ...
            const int  pair = std::clamp (ch.outputPair.load (std::memory_order_relaxed), 0, numPairs - 1);
            snap[(size_t) c] = { ch.gain.load (std::memory_order_relaxed), muted || silencedBySolo, pair };
        }
